...
def create_image_db(db_name='universal_image_archive.db'):
    ...
    # Check and add XML metadata columns if they don't exist
...
def download_and_store_file(conn, image_url, xml_url):
    ...
    try:
        ...
        # Check if file is an image using Pillow
        try:
            image = Image.open(io.BytesIO(image_blob_data))
            print(f"🖼️  File is a valid {image.format} image: {image.size[0]}x{image.size[1]} pixels")
        except:
        ...
        # Insert the file data and XML metadata
        cursor.execute('''
...
def display_image_if_available(conn, file_id):
    ...
    try:
        ...
        # Save the original file for inspection
...
def main():
    ...
    IMAGE_URL = 'https://archive.org/download/air-canada-767/Air Canada 767.jpg'
    XML_URL = 'https://archive.org/download/air-canada-767/air-canada-767_meta.xml'
...
#sqlite3 universal_image_archive.db
#SELECT * FROM archived_files;
#.schema archived_files
#https://archive.org/details/digitaltransportation_postcards?page=8