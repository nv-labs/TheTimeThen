...
def intersperse_images(keyword_images, non_keyword_images, max_images=35, min_gap=2):
    ...
    # Calculate ideal positions for keyword images to spread them evenly
    if keyword_count > 0:
        # Spread keyword images across the total_images length
        step = max(min_gap, total_images // (keyword_count + 1))
        keyword_positions = [step * (i + 1) for i in range(keyword_count)]
        # Adjust positions to ensure they don't exceed total_images - 1
        ...
        # Trim positions to fit within total_images
    ...
    for i in range(total_images):
        if keyword_idx < keyword_count and i == keyword_positions[keyword_idx]:
            ...
            # Fallback to keyword images if non-keyword images run out
            result.append(keyword_images[keyword_idx])
            keyword_idx += 1
    
    # Ensure we have exactly total_images
    result = result[:total_images]
    
    # If we still have keyword images left, replace some non-keyword images
    while keyword_idx < keyword_count and len(result) < total_images:
        result.append(keyword_images[keyword_idx])
        keyword_idx += 1
