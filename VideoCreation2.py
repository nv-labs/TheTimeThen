...
def create_image_db(db_name='universal_image_archive.db', schema_name='photos_2023'):
    """Creates or connects to the photos table with a schema-like prefix."""
    ...
    if 'VideoAirDate' not in columns:
        cursor.execute(f'ALTER TABLE {table_name} ADD COLUMN VideoAirDate TEXT')
...
def process_regular_image(row_id, file_data, title, target_width, target_height, output_dir, save_count, max_save=20):
    """Process a regular image from DB data: background, stars, enlarge if small, center, title overlay for video."""
    try:
...
def intersperse_images(keyword_images, non_keyword_images, max_images=35, min_gap=2):
    """Intersperse keyword images evenly with non-keyword images to avoid consecutive keyword images."""
    result = []
...
def main(output_dir):
    ...
    OUTRO_IMAGE = r"D:\Dev\TheTimeThen\VideoAssets\End Screen V2.png"
    SCHEMA_NAME = 'photos_2023'
    ...
    try:
        conn = create_image_db(DATABASE_NAME, SCHEMA_NAME)
        cursor = conn.cursor()
        table_name = f"{SCHEMA_NAME}_photos"
        ...
        if os.path.exists(INTRO_IMAGE):
            with open(INTRO_IMAGE, 'rb') as f:
                intro_frames = process_intro_outro_image(io.BytesIO(f.read()), 1920, 1088, num_frames_intro, fps, is_intro=True)
        ...
        if os.path.exists(OUTRO_IMAGE):
            with open(OUTRO_IMAGE, 'rb') as f:
                outro_frames = process_intro_outro_image(io.BytesIO(f.read()), 1920, 1088, num_frames_outro, fps, is_intro=False)
            print("Processed outro image (animated stars) from file system.")
        else:
            print(f"Outro image not found at {OUTRO_IMAGE}.")
        ...
        cursor.execute(f"""
            ...
            AND (VideoAirDate IS NULL OR VideoAirDate = '')
        ...
        cursor.execute(f"""
            ...
            AND xml_title NOT LIKE '%Michigan%'
            AND xml_title NOT LIKE '%Bette%'
            AND xml_title NOT LIKE '%Barbara%'
            AND xml_title NOT LIKE '%Rochelle%'
            AND xml_title NOT LIKE '%Virginia%'
            ORDER BY id DESC
            LIMIT 34
        """)
        ...
        keywords = [
            ...
            'Chicago', 'Ohio', 'Oklahoma', 'Florida', 'Michigan'
        ]
        
        print("Selected image titles:")
        for i, row in enumerate(rows):
            title = row[2]
            is_keyword = any(keyword.lower() in title.lower() for keyword in keywords)
            print(f"Image {i+1} (ID {row[0]}): {title} [{'Keyword' if is_keyword else 'Non-keyword'}]")
        ...
        for i, row in enumerate(rows):
            ...
            if img_array is not None:
                ...
                cursor.execute(f'UPDATE {table_name} SET VideoAirDate = ? WHERE id = ?', (current_timestamp, proc_id))
                conn.commit()
        ...
        if image_arrays or intro_frames or outro_frames:
            ...
            try:
                ...
                video_stream = ffmpeg.input(temp_video).video
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 2:
        print("Usage: python videoCreation-Long-2023.py <output_directory>")
        sys.exit(1)
    main(sys.argv[1])